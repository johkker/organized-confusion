...
const DJModal = ({ dj, onClose }: DJModalProps) => {
  ...
  return (
    <div className="dj-modal" role="dialog" aria-modal="true">
      <div className="dj-modal-content" ref={modalRef}>
        <button 
...
const DJs = ({ djNames }: DJsProps) => {
  const { djs, loading, error, fetchDJs } = useDJStore()
  const [selectedDJ, setSelectedDJ] = useState<DJ | null>(null)
  const [displayDJs, setDisplayDJs] = useState<DJ[]>([])
  const [selectedGenre, setSelectedGenre] = useState<string | null>(null)
  const [availableGenres, setAvailableGenres] = useState<string[]>([])
  const [filteredByNames, setFilteredByNames] = useState<DJ[]>([])
  ...
  // First filter DJs based on provided names - this creates our base filtered list
  useEffect(() => {
    const filtered = djNames && djNames.length > 0
      ? djs.filter(dj => djNames.includes(dj.name))
      : djs
    setFilteredByNames(filtered)

    // Extract all unique genres from the name-filtered DJs
    ...
    // Reset genre selection when DJ names change
  ...
  useEffect(() => {
    if (selectedGenre) {
      setDisplayDJs(filteredByNames.filter(dj => dj.genres.includes(selectedGenre)))
    } else {
      setDisplayDJs(filteredByNames)
    }
  }, [selectedGenre, filteredByNames])

  if (loading) {
    return <div className="loading-container">Loading DJs...</div>;
  }
